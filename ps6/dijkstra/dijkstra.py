...
class PathFinder(object):
    ...
    def dijkstra(self, weight, nodes, source, destination):
        ...
        node_number = 0
        while len(queue) > 0:
            u_pair = queue.extract_min()
            node_number += 1
            u = u_pair.node
            if u is destination:
                break
            for v in u.adj:
                next_dist = u_pair.distance + weight(u, v)
                if v.queue_key is None:
                    v.queue_key = NodeDistancePair(v, next_dist)
                    queue.insert(v.queue_key)
                    v.parent = u
                elif next_dist < v.queue_key.distance:
                    v.queue_key.distance = next_dist
