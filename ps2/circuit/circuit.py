...
class PriorityQueue:
    ...
    def append(self, key):
        ...
        # minimize from the key just appended
        self.min_heapify(len(self.heap)-1)  

    # running time should be O(lgn), lgn is heap height
    def min_heapify(self, index):
        if index == 0:
            return 
        parent = (index-1) / 2      # careful
        if self.heap[index] < self.heap[parent]:
            # exchange parent with index
    ...
    def pop(self):
        ...
        if len(self.heap) == 0:
            return None
        popped_key = self.heap[0]
        self.heap[0] = self.heap[-1]    # last element to first
        self.heap = self.heap[:-1]      # discard last

        heap_len = len(self.heap)
        i = 0
        # print "heap length " + str(heap_len)
        # min_heapify 
        while i < heap_len/2:
            # print i
            ...
            if 2*(i+1) < heap_len:
                ...
                right = self.heap[-1]   # trick    
            current = self.heap[i]      

            if (current > left or current > right) and (left <= right):
                self.heap[i] = left
                self.heap[2*(i+1)-1] = current
                i = 2*(i+1)-1
            elif (current > left or current > right) and (left > right):
                self.heap[i] = right
                self.heap[2*(i+1)] = current
                i = 2*(i+1)
            else:
                break
